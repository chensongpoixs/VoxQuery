...
class Reranker:
    ...
    对检索结果进行精排，综合考虑：
    - 语义相似度（Embedding 距离）
    - 关键词覆盖度（行业术语匹配，可自定义权重）
    - 文档来源权威性（正式文档 > 草稿）
    - 文本完整性（完整段落 > 截断片段）
    """

    # 企业知识库核心关键词权重（可根据行业自定义）
    KEYWORDS_WEIGHTS = {
        "安全": 0.10,
        "操作": 0.08,
        "流程": 0.10,
    ...
    def _authority_score(self, metadata: Dict) -> float:
        ...
        score = 0.5  # 基础分
        if any(kw in combined for kw in ["标准", "流程", "规范", "手册", "制度"]):
            score += 0.3
        if any(kw in combined for kw in ["iso", "国标", "认证", "合规"]):
            score += 0.2
